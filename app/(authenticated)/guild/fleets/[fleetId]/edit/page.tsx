...
export default function EditFleetPage() {
  ...
  useEffect(() => {
    const fetchFleet = async () => {
      if (!selectedPlayer || !fleetId) return;

      try {
        const [fleetRes, tagsRes] = await Promise.all([
          squadsApi.get(selectedPlayer.guildId, fleetId),
          squadTagsApi.list(selectedPlayer.guildId),
        ]);

        const fleet = fleetRes.squad;
        setName(fleet.name);
        setDescription(fleet.description || '');
        setSelectedTagIds(fleet.tags.map(t => t.id));
        setTags(tagsRes.tags);

        // Convert fleet slots to form data
        const newSlots: SlotFormData[] = Array.from({ length: 8 }, (_, i) => ({
          position: i,
          minRarity: 1,
          minCrewRelicLevel: 0,
          ship: null,
        ...
        for (const slot of fleet.slots) {
          if (slot.position >= 0 && slot.position < 8 && slot.ships.length > 0) {
            const shipData = slot.ships[0].ship;
            newSlots[slot.position] = {
              position: slot.position,
              minRarity: slot.minRarity || 1,
              minCrewRelicLevel: slot.minCrewRelicLevel || 0,
              ship: {
                baseId: shipData.baseId,
                name: shipData.name,
                thumbnailName: shipData.thumbnailName,
                isCapital: slot.position === 0,
                categories: [],
                crew: [],
                abilities: [],
              },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await squadsApi.update(selectedPlayer.guildId, fleetId, {
        name: name.trim(),
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Edit Fleet</h1>
        <button
          onClick={() => router.push('/guild/fleets')}
        ...
        {slot.ship ? (
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              {/* Crew info */}
              {slot.ship.crew && slot.ship.crew.length > 0 && (
