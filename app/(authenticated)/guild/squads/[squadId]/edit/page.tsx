...
export default function EditSquadPage() {
  ...
  useEffect(() => {
    const fetchSquad = async () => {
      if (!selectedPlayer || !squadId) return;

      try {
        const [squadRes, tagsRes, categoriesRes] = await Promise.all([
          squadsApi.get(selectedPlayer.guildId, squadId),
        ...
        setSelectedTagIds(squad.tags.map(t => t.id));
        setTags(tagsRes.tags);
        setCategories(categoriesRes.categories);

        // Collect all character IDs to fetch their full data
        const characterIds: string[] = [];
        for (const slot of squad.slots) {
          for (const char of slot.characters) {
            if (!characterIds.includes(char.character.baseId)) {
              characterIds.push(char.character.baseId);
            }
          }
        }

        // Fetch full character data (with abilities) for all characters
        const characterDataMap: Record<string, CharacterSummary> = {};
        await Promise.all(
          characterIds.map(async (baseId) => {
            try {
              const res = await charactersApi.get(baseId);
              characterDataMap[baseId] = res.character;
            } catch (err) {
              console.error(`Failed to fetch character ${baseId}:`, err);
            }
          })
        );

        // Convert squad slots to form data
        const newSlots: SlotFormData[] = [
          { ...EMPTY_SLOT, position: 0 },
          { ...EMPTY_SLOT, position: 1 },
          { ...EMPTY_SLOT, position: 2 },
          { ...EMPTY_SLOT, position: 3 },
        ...
        for (const slot of squad.slots) {
          if (slot.position >= 0 && slot.position < 5) {
            newSlots[slot.position] = {
              position: slot.position,
              slotType: slot.slotType,
              categoryMatchMode: slot.categoryMatchMode || 'all',
              minRarity: slot.minRarity || 1,
              minGearLevel: slot.minGearLevel || 1,
              minRelicLevel: slot.minRelicLevel || 0,
              categoryIds: slot.categories.map(c => c.id),
              characters: slot.characters.map(c => ({
                character: characterDataMap[c.character.baseId] || {
                  baseId: c.character.baseId,
                  name: c.character.name,
                  thumbnailName: c.character.thumbnailName,
                  isLeader: false,
                  isGalacticLegend: false,
                  categories: [],
                  abilities: [],
                },
                requiredZetas: c.requiredZetas || [],
                requiredOmicrons: c.requiredOmicrons || [],
              })),
            };
          }
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await squadsApi.update(selectedPlayer.guildId, squadId, {
        name: name.trim(),
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Edit Squad</h1>
        <button
          onClick={() => router.push('/guild/squads')}
                    ...
                    {slot.characters.length > 0 ? (
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          {/* Zetas and Omicrons - show if character has abilities loaded */}
                          {slot.characters[0].character.abilities?.length > 0 &&
                          ...
                          {/* Show existing zeta/omicron requirements */}
                          {(slot.characters[0].requiredZetas.length > 0 || slot.characters[0].requiredOmicrons.length > 0) && (
                            <div className="mt-2 flex flex-wrap gap-1">
                              {slot.characters[0].requiredZetas.map(z => (
                                <span key={z} className="px-2 py-0.5 text-xs bg-purple-600 rounded">Z: {z}</span>
                              ))}
                              {slot.characters[0].requiredOmicrons.map(o => (
                                <span key={o} className="px-2 py-0.5 text-xs bg-blue-600 rounded">O: {o}</span>
                              ))}
                            </div>
                          )}
                        </div>
