...
export default function CharactersPage() {
  ...
  const [hasZeta, setHasZeta] = useState(false);
  const [hasOmicron, setHasOmicron] = useState(false);
  ...
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, alignment, role, faction, glOnly, hasZeta, hasOmicron]);
  ...
  const fetchCharacters = useCallback(async () => {
    ...
    try {
      const response = await charactersApi.list({
        ...
        isGalacticLegend: glOnly || undefined,
        hasZeta: hasZeta || undefined,
        hasOmicron: hasOmicron || undefined,
      });
  ...
  }, [page, debouncedSearch, alignment, role, faction, glOnly, hasZeta, hasOmicron]);
  ...
  const hasActiveFilters = search || alignment || role || faction || glOnly || hasZeta || hasOmicron;
  ...
  const countZetas = (abilities: CharacterSummary['abilities']) =>
    abilities.filter(a => a.hasZeta).length;

  const countOmicrons = (abilities: CharacterSummary['abilities']) =>
        ...
        <div className="flex flex-wrap items-center gap-4 mt-4">
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input
              type="checkbox"
              checked={glOnly}
              onChange={e => setGlOnly(e.target.checked)}
              className="w-4 h-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500"
            />
            <span className="text-gray-300">Galactic Legends only</span>
          </label>
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input
              type="checkbox"
              checked={hasZeta}
              onChange={e => setHasZeta(e.target.checked)}
              className="w-4 h-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500"
            />
          ...
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input
              type="checkbox"
              checked={hasOmicron}
              onChange={e => setHasOmicron(e.target.checked)}
              className="w-4 h-4 rounded border-gray-600 bg-gray-700 text-indigo-600 focus:ring-indigo-500"
            />
          ...
          <table className="w-full">
            <thead className="bg-gray-700">
              <tr>
                ...
                <th className="px-4 py-3 text-center text-sm font-semibold">GL</th>
                ...
                characters.map(char => (
                  <tr key={char.baseId} className="hover:bg-gray-750">
                    <td className="px-4 py-3">
                      <Link
                        href={`/dashboard/game-data/characters/${char.baseId}`}
                    ...
                    <td className="px-4 py-3">{getAlignmentBadge(char.categories)}</td>
                    <td className="px-4 py-3">{getRoleBadge(char.categories)}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {getFactions(char.categories).slice(0, 3).map(f => (
                          <span
                            key={f.id}
                        ...
                        {getFactions(char.categories).length > 3 && (
                    ...
                    <td className="px-4 py-3 text-center text-sm text-gray-400">
                      {countZetas(char.abilities) || '-'}
                    </td>
                    <td className="px-4 py-3 text-center text-sm text-gray-400">
                      {countOmicrons(char.abilities) || '-'}
                    </td>
                  </tr>
                ))
              )}
