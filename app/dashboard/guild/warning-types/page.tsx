...
import { warningsApi, WarningType } from '../../../../lib/api';
...
export default function WarningTypesPage() {
  ...
  const [formData, setFormData] = useState<FormData>({ name: '', severity: 5 });
  ...
  const handleAddClick = () => {
    setFormMode('add');
    setEditingId(null);
    setFormData({ name: '', severity: 5 });
  };

  const handleEditClick = (type: WarningType) => {
    setFormMode('edit');
    setEditingId(type.id);
    setFormData({ name: type.name, severity: type.severity });
  };
