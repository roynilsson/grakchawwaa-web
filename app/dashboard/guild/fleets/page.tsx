...
export default function FleetsPage() {
  ...
  const getCapitalShipThumbnail = (fleet: Squad): string | null => {
    const capitalSlot = fleet.slots.find((slot) => slot.position === 0);
    if (capitalSlot && capitalSlot.ships.length > 0) {
      return capitalSlot.ships[0].ship.thumbnailName;
    }
    return null;
  };

  const getCapitalShipName = (fleet: Squad): string | null => {
    const capitalSlot = fleet.slots.find((slot) => slot.position === 0);
    if (capitalSlot && capitalSlot.ships.length > 0) {
      return capitalSlot.ships[0].ship.name;
    }
    return null;
  };
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => router.push('/dashboard/guild/fleets/new')}
      ...
      {fleets.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {fleets.map((fleet) => {
            const capitalThumbnail = getCapitalShipThumbnail(fleet);
            const capitalName = getCapitalShipName(fleet);
            ...
            return (
              ...
              >
                <div className="flex items-start gap-4">
                  {/* Capital Ship Thumbnail */}
                  <div className="flex-shrink-0">
                    {capitalThumbnail ? (
                      <Image
                        src={getUnitThumbnail(capitalThumbnail)}
                        alt={capitalName || 'Capital Ship'}
                        width={64}
                        height={64}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-semibold text-lg truncate">{fleet.name}</h3>
                      {fleet.isTemplate && (
                    ...
                    {capitalName && (
                      <p className="text-sm text-gray-400 mb-2">Capital: {capitalName}</p>
                    )}
                    {fleet.tags.length > 0 && (
                ...
                {!fleet.isTemplate && (
                  <div className="mt-4 pt-4 border-t border-gray-700 flex justify-end gap-2">
                    {deleteConfirmId === fleet.id ? (
                      ...
                      <>
                        <button
                          onClick={() => router.push(`/dashboard/guild/fleets/${fleet.id}/edit`)}
              ...
              {templates.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {templates.map((template) => {
                    const capitalThumbnail = getCapitalShipThumbnail(template);
                    const capitalName = getCapitalShipName(template);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {capitalName && (
                            <p className="text-sm text-gray-400">Capital: {capitalName}</p>
                          )}
